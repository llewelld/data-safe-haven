...
until su-exec "$USER" /usr/local/bin/gitea admin user list | grep "{{mirror_username}}" > /dev/null 2>&1; do
    ...
    su-exec "$USER" /usr/local/bin/gitea admin user create --username "{{mirror_username}}" --password "{{mirror_password}}" --must-change-password=false --email "{{mirror_email}}" 2> /dev/null
    sleep 1
done
...
su-exec "$USER" /usr/local/bin/gitea admin user change-password  --username "{{mirror_username}}" --password "{{mirror_password}}" --must-change-password=false